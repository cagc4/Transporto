...
class Companion
{
	...
	function addCompanion($companionData)	{
		...
		$this->result = $this->util->db->Execute("INSERT INTO cc_acompanante_tbl VALUES('".$companionBasicData->docType."',
			...
			if(!$this->result) {
				$this->util->db->Execute("DELETE FROM cc_acompanante_tbl WHERE cc_tipo_doc_fld = '".$companionBasicData->docType."' AND cc_nume_doc_fld = '".$companionBasicData->docNum."'");
				$respCode = 1;
			}
		return $respCode;
	}
	...
	function modifyCompanion($companionData, $docNum, $docType) {
		$respCode = 0;
		$this->util->db->Execute("DELETE FROM cc_acompanante_tbl WHERE cc_nume_doc_fld = '".$docNum."' AND cc_tipo_doc_fld = (SELECT cc_valor_fld FROM cc_valores_tbl WHERE cc_descripcion_fld = '".$docType."' AND cc_campo_fld = 'cc_tipo_doc_fld')");
