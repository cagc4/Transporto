...
CREATE TABLE `cc_formocacional_tbl` (
  ...
  PRIMARY KEY (`cc_id_fld`)
) ENGINE=MyISAM AUTO_INCREMENT=2233 DEFAULT CHARSET=latin1;
...
CREATE TABLE `cc_fuec_ocupantes_tbl` (
  ...
  `cc_id_ocupante_fld` int(11) DEFAULT NULL,
...
CREATE TABLE `cc_fuec_tbl` (
  ...
  `cc_nombre_responsable_fld` varchar(255) DEFAULT NULL,
...
DROP VIEW IF EXISTS `cc_fuec_ocupantes_vw`;
CREATE VIEW `cc_fuec_ocupantes_vw` AS 
  select `cc_fuec_ocupantes_tbl`.`cc_numero_fuec_fld` AS `FUEC`,(select `cc_valores_tbl`.`cc_descripcion_fld` from `cc_valores_tbl` where ((`cc_valores_tbl`.`cc_campo_fld` = 'cc_tipo_doc_fld') and (`cc_valores_tbl`.`cc_valor_fld` = `cc_fuec_ocupantes_tbl`.`cc_tipo_doc_fld`))) AS `Tipo_Identificacion`,`cc_fuec_ocupantes_tbl`.`cc_num_id_fld` AS `Numero_Identificacion`,`cc_fuec_ocupantes_tbl`.`cc_nombre_fld` AS `Nombre` from `cc_fuec_ocupantes_tbl`;

DROP VIEW IF EXISTS `cc_fuec_vw`;
CREATE VIEW `cc_fuec_vw` AS 
  select `fc`.`cc_numero_fuec_fld` AS `Numero_FUEC`,`fr`.`cc_id_fld` AS `Numero_Contrato`,`fr`.`cc_objetoCont_fld` AS `Objeto_Contrato`,`fr`.`cc_placa_fld` AS `Placa`,`fr`.`cc_fechaSali_fld` AS `Fecha_Salida`,`fr`.`cc_destino_fld` AS `Destino` from (`cc_fuec_tbl` `fc` join `cc_contract_tbl` `fr` on((`fc`.`cc_num_contrato_tbl` = `fr`.`cc_id_fld`))) order by substr(`fc`.`cc_numero_fuec_fld`,18,4) desc;
...
CREATE VIEW `cc_valores_vw` AS 
  select `b`.`cc_descripcion_fld` AS `Campo`,`a`.`cc_valor_fld` AS `Valor`,`a`.`cc_descripcion_fld` AS `Descripcion`,`a`.`cc_estado_fld` AS `Estado` from (`cc_valores_tbl` `a` join `cc_fields_tbl` `b`) where ((`a`.`cc_campo_fld` = `b`.`cc_campo_fld`) and (`a`.`cc_estado_fld` = 'A'));
...
DROP VIEW IF EXISTS `cc_warnings_vw`;
CREATE VIEW `cc_warnings_vw` AS 
  select (case when ((to_days(`a`.`cc_fecha_ven_fld`) - to_days(curdate())) < 0) then concat((to_days(`a`.`cc_fecha_ven_fld`) - to_days(curdate())),' dias V') when (((to_days(`a`.`cc_fecha_ven_fld`) - to_days(curdate())) >= 1) and ((to_days(`a`.`cc_fecha_ven_fld`) - to_days(curdate())) <= 30)) then concat((to_days(`a`.`cc_fecha_ven_fld`) - to_days(curdate())),' dias PV') else 0 end) AS `Estado`,(select `cc_valores_tbl`.`cc_descripcion_fld` from `cc_valores_tbl` where ((`cc_valores_tbl`.`cc_campo_fld` = 'cc_tipo_docum_fld') and (`cc_valores_tbl`.`cc_valor_fld` = `a`.`cc_tipo_docum_fld`))) AS `Documento`,`a`.`cc_identificador_fld` AS `Numero`,`a`.`cc_placa_fld` AS `Placa`,concat(ucase(`c`.`cc_tipo_doc_fld`),'-',`c`.`cc_nume_doc_fld`) AS `Identificacion`,`a`.`cc_fecha_ven_fld` AS `Vencimiento` from (((`cc_documento_tbl` `a` join `cc_per_veh_tbl` `b`) join `cc_propcond_tbl` `c`) join `cc_person_tbl` `d`) where ((`a`.`cc_placa_fld` = `b`.`cc_placa_fld`) and (`b`.`cc_tipo_doc_fld` = `c`.`cc_tipo_doc_fld`) and (`b`.`cc_nume_doc_fld` = `c`.`cc_nume_doc_fld`) and (`c`.`cc_tipo_doc_fld` = `d`.`cc_tipo_doc_fld`) and (`c`.`cc_nume_doc_fld` = `d`.`cc_nume_doc_fld`) and (`c`.`cc_type_pc_fld` = 1)) having (((to_days(`a`.`cc_fecha_ven_fld`) - to_days(curdate())) >= 1) and ((to_days(`a`.`cc_fecha_ven_fld`) - to_days(curdate())) <= 7)) order by 1;
